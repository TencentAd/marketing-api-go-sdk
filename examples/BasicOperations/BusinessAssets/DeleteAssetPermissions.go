/*
 * Marketing API
 *
 * Marketing API
 *
 * API version: 1.3
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */

package main

import (
	"encoding/json"
	"fmt"
	"net/http"

	"github.com/tencentad/marketing-api-go-sdk/pkg/ads"
	"github.com/tencentad/marketing-api-go-sdk/pkg/config"
	"github.com/tencentad/marketing-api-go-sdk/pkg/errors"
	"github.com/tencentad/marketing-api-go-sdk/pkg/model"
)

type AssetPermissionsDeleteExample struct {
	TAds        *ads.SDKClient
	AccessToken string
	Data        model.AssetPermissionsDeleteRequest
}

func (e *AssetPermissionsDeleteExample) Init() {
	e.AccessToken = "YOUR ACCESS TOKEN"
	e.TAds = ads.Init(&config.SDKConfig{
		AccessToken: e.AccessToken,
		IsDebug:     true,
	})
	e.Data = model.AssetPermissionsDeleteRequest{
		AccountId:                int64(0),
		AssetPermissionGrantType: model.AssetPermissionGrantType_ASSET,
		AssetType:                model.AssetType_CANVAS_WECHAT,
		AssetId:                  int64(0),
		LicensingAccountId:       int64(0),
	}
}

func (e *AssetPermissionsDeleteExample) RunExample() (interface{}, http.Header, error) {
	tads := e.TAds
	// change ctx as needed
	ctx := *tads.Ctx
	return tads.AssetPermissions().Delete(ctx, e.Data)
}

func main() {
	e := &AssetPermissionsDeleteExample{}
	e.Init()
	response, headers, err := e.RunExample()
	if err != nil {
		if resErr, ok := err.(errors.ResponseError); ok {
			errStr, _ := json.Marshal(resErr)
			fmt.Println("Response error:", string(errStr))
		} else {
			fmt.Println("Error:", err)
		}
	}
	fmt.Println("Response data:", response)
	fmt.Println("Headers:", headers)
}
