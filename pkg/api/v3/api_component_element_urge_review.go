...
func (a *ComponentElementUrgeReviewApiService) Add(ctx context.Context, data ComponentElementUrgeReviewAddRequest) (ComponentElementUrgeReviewAddResponseData, http.Header, error) {
	var (
		...
		localVarReturnValue ComponentElementUrgeReviewAddResponseData
		localVarResponse    ComponentElementUrgeReviewAddResponse
	)

	// create path and map variables
	localVarPath := a.client.Cfg.BasePath + "/component_element_urge_review/add"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v ComponentElementUrgeReviewAddResponse
...
/*
ComponentElementUrgeReviewApiService 获取创意组件元素催审状态
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param accountId
 * @param dynamicCreativeId
 * @param optional nil or *ComponentElementUrgeReviewGetOpts - Optional Parameters:
     * @param "ComponentIdList" (optional.Interface of []int64) -
     * @param "ElementFingerprintList" (optional.Interface of []string) -
...
func (a *ComponentElementUrgeReviewApiService) Get(ctx context.Context, accountId int64, dynamicCreativeId int64, localVarOptionals *ComponentElementUrgeReviewGetOpts) (ComponentElementUrgeReviewGetResponseData, http.Header, error) {
	var (
		...
		localVarFileBytes   []byte
		localVarFileKey     string
		localVarReturnValue ComponentElementUrgeReviewGetResponseData
		localVarResponse    ComponentElementUrgeReviewGetResponse
	)

	// create path and map variables
	localVarPath := a.client.Cfg.BasePath + "/component_element_urge_review/get"
	...
	if localVarOptionals != nil && localVarOptionals.ComponentIdList.IsSet() {
		localVarQueryParams.Add("component_id_list", parameterToString(localVarOptionals.ComponentIdList.Value(), "multi"))
	}
	if localVarOptionals != nil && localVarOptionals.ElementFingerprintList.IsSet() {
		localVarQueryParams.Add("element_fingerprint_list", parameterToString(localVarOptionals.ElementFingerprintList.Value(), "multi"))
	}
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v ComponentElementUrgeReviewGetResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse.Header, newErr
