...
func (a *BatchAsyncRequestsApiService) Add(ctx context.Context, data BatchAsyncRequestsAddRequest) (BatchAsyncRequestsAddResponseData, http.Header, error) {
	var (
		...
		localVarReturnValue BatchAsyncRequestsAddResponseData
		localVarResponse    BatchAsyncRequestsAddResponse
	)

	// create path and map variables
	localVarPath := a.client.Cfg.BasePath + "/batch_async_requests/add"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v BatchAsyncRequestsAddResponse
...
/*
BatchAsyncRequestsApiService 获取批量异步请求任务列表
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param accountId
...
func (a *BatchAsyncRequestsApiService) Get(ctx context.Context, accountId int64, localVarOptionals *BatchAsyncRequestsGetOpts) (BatchAsyncRequestsGetResponseData, http.Header, error) {
	var (
		...
		localVarFileBytes   []byte
		localVarFileKey     string
		localVarReturnValue BatchAsyncRequestsGetResponseData
		localVarResponse    BatchAsyncRequestsGetResponse
	)

	// create path and map variables
	localVarPath := a.client.Cfg.BasePath + "/batch_async_requests/get"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v BatchAsyncRequestsGetResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse.Header, newErr
